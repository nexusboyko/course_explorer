...
class GSConnection():
    ...
    def get_account(self):
        ...
        instructor_courses = parsed_account_resp.find('h1', class_ ='pageHeading')
    ...
    def get_account_alt(self) -> list[Tag]:

        if self.state != ConnState.LOGGED_IN:
            return False # Should raise exception
        # Get account page and parse it using bs4
        account_resp: requests.Response  = self.session.get("https://www.gradescope.com/account")
        parsed_account_resp = BeautifulSoup(account_resp.text, 'html.parser')
        ...
        if instructor_courses:
            ...
            instructor_courses = None
        courses_for_term: NavigableString = parsed_account_resp.find('div', class_ = 'courseList--coursesForTerm')
        tags: list[Tag]= []
        if courses_for_term:
            # print(courses_for_term)
            course: NavigableString
            for course in courses_for_term:
                tag = course.find('h3', class_ ='courseBox--shortname')
                url: Tag = course.find('a', class_ = 'courseBox')
                print(url)
                if tag:
                    tags.append(tag)
        return tags
